...
DriveStats::DriveStats(QWidget* parent) : QWidget(parent) {
  ...
  auto add_stats_layouts = [&](QGridLayout* gl, StatsLabels& labels, int row, const QString &distance_unit) {
    gl->addLayout(build_stat_layout(&labels.routes, "DRIVES"), row, 0, 3, 1); //주행수
    gl->addLayout(build_stat_layout(&labels.distance, distance_unit), row, 1, 3, 1);
    gl->addLayout(build_stat_layout(&labels.hours, "HOURS"), row, 2, 3, 1); //시간
  };

  QGridLayout* gl = new QGridLayout(this);
  gl->setMargin(0);

  gl->addWidget(new QLabel("ALL TIME"), 0, 0, 1, 3); //전체시간
  add_stats_layouts(gl, all_, 1, distance_unit);

  gl->addWidget(new QLabel("PAST WEEK"), 6, 0, 1, 3); //지난주
